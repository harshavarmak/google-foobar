...
def solution(bunnies, keyRequired):
    keys = [[] for bun in range(bunnies)]
    copies = bunnies - keyRequired + 1
    counter = 0
    for bunny in combinations(range(bunnies), copies):
        for bun in bunny:
            keys[bun].append(counter)
        counter += 1
    return keys